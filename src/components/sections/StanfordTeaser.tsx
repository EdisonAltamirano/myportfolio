        ...
        <FadeIn>
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
            <div>
              ...
              <p className="font-mono text-sm text-sky-400 tracking-wide">
                2024–Present · Arbabian Lab · TSMC Tape-out
              </p>
            </div>
            <Link
              href="/stanford"
              className="inline-flex items-center gap-2 font-display text-sm font-semibold tracking-wider uppercase text-sky-400 hover:text-sky-300 border border-sky-500/35 hover:border-sky-500/60 px-6 py-3 rounded-sm transition-all duration-300 group shrink-0"
            >
            ...
            {tracks.map((track, i) => {
              ...
              return (
                <FadeIn key={track.title} delay={`delay-${i * 75}`}>
                  <div className={`rounded-lg border ${track.border} bg-card/50 p-5 hover:bg-card/70 transition-all duration-300 group`}>
          ...
          <div className="mt-8 flex flex-wrap gap-6 items-center justify-center md:justify-start border-t border-border/30 pt-6">
            {[
              ...
              <div key={s.label} className="text-center md:text-left">
                <div className="font-display text-sm font-bold text-sky-400">{s.label}</div>
                <div className="font-mono text-[10px] text-muted-foreground/50 tracking-wide mt-0.5">{s.sub}</div>
              </div>
            ))}
          </div>
        </FadeIn>
